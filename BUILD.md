...
## Package
`cloud-dog-idam` - identity, authentication, and role management helpers.
...
```bash
python -c "import cloud_dog_idam; print('package import ok')"
```

## Run Tests
```bash
ruff check cloud_dog_idam tests
ruff format --check cloud_dog_idam tests
...
python -m pytest tests/security --env ./.env.test -v
```
...
docker build -t cloud-dog-idam:latest .
```

## Dependencies
- runtime extras for OIDC, LDAP, SAML, MFA, and FastAPI integration are declared in `pyproject.toml`
