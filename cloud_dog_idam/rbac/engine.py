...
class RBACEngine:
    ...
    def _invalidate_user(self, user_id: str) -> None:
        """Invalidate the cached roles AND permissions for a user.

        W28A-820 fix: effective roles/permissions are cached under the prefixed
        keys ``roles:{user_id}`` / ``perms:{user_id}`` (see get_effective_*), so
        invalidating the bare ``user_id`` key left the real entries stale until
        TTL — role/group changes (membership propagation, downgrades) did not
        take effect promptly. Clear the actual prefixed keys.
        """
