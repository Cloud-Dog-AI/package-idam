...
class ResourceTypeRepository(BaseRepository[m.ResourceTypeORM]):
    """Represent resource type repository."""
    orm_model = m.ResourceTypeORM


class ResourceInstanceRepository(BaseRepository[m.ResourceInstanceORM]):
    """Represent resource instance repository."""
    orm_model = m.ResourceInstanceORM


class RBACBindingRepository(BaseRepository[m.RBACBindingORM]):
    """Represent RBAC binding repository."""
    orm_model = m.RBACBindingORM

    def by_subject(self, subject_type: str, subject_id: str) -> list[m.RBACBindingORM]:
        """Return bindings for a user or group subject."""
        return list(
            self._session.scalars(
                select(m.RBACBindingORM).where(
                    m.RBACBindingORM.subject_type == subject_type,
                    m.RBACBindingORM.subject_id == subject_id,
                )
            )
        )
