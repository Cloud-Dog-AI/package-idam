...
class ResourceTypeORM(Base):
    """Represent registered project resource types."""
    __tablename__ = "resource_types"
    resource_type_id: Mapped[str] = mapped_column(String(64), primary_key=True)
    project: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    type: Mapped[str] = mapped_column(String(128), nullable=False)
    label: Mapped[str] = mapped_column(String(256), nullable=False)
    permissions: Mapped[list] = mapped_column(JSON, default=list)
    list_endpoint: Mapped[str | None] = mapped_column(String(512), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=_utcnow
    )
...
class ResourceInstanceORM(Base):
    """Represent a concrete registered resource instance."""
    __tablename__ = "resource_instances"
    resource_instance_id: Mapped[str] = mapped_column(String(64), primary_key=True)
    project: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
...
class RBACBindingORM(Base):
    """Represent explicit user/group resource permission bindings."""
    ...
    granted_by: Mapped[str] = mapped_column(String(64), nullable=False, default="system")
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=_utcnow
    )
